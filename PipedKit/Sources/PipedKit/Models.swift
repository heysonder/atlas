...
public struct Stream: Codable, Hashable, Sendable {
    ...
    /// The codec-less fallback must be a video mime type so "audio/mp4" doesn't match.
...
public struct VideoDetail: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.videoStreams = try values.decodeLossyArray([Stream].self, forKey: .videoStreams)
        self.audioStreams = try values.decodeLossyArray([Stream].self, forKey: .audioStreams)
    ...
    /// Word-boundary match of a language code (or its English display name)
    /// against a subtitle's display name, so "en" matches "English" but never
    /// substring-matches inside "French".
...
public struct Channel: Codable, Sendable {
    ...
    enum CodingKeys: String, CodingKey {
        case id, name, avatarUrl, bannerUrl, description, nextpage
...
public struct CommentsPage: Codable, Sendable {
    ...
    enum CodingKeys: String, CodingKey {
        case comments, nextpage, disabled, commentCount
    }
...
public struct SearchResponse: Codable, Sendable {
    ...
    enum CodingKeys: String, CodingKey {
        case items, nextpage, suggestion, corrected
    }
